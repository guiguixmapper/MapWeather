    ...
    for cp in resultats_meteo:
        ...
        if pluie_pct is not None:
            ...
            barre_pluie = (
                ...
                f'<div style="background:#e5e7eb;border-radius:4px;height:7px;width:100%">'
                f'<div style="background:{pluie_couleur};width:{pluie_pct}%;height:7px;'
                f'border-radius:4px;transition:width .3s"></div></div>'
            )
        else:
        ...
        <div style="font-family:sans-serif;font-size:12px;min-width:200px">
          ...
          <div style="color:#6b7280;margin-bottom:6px">⛰️ Alt : {cp['Alt (m)']} m</div>

          <div style="font-size:15px;margin-bottom:6px">{cp['Ciel']} <b>{t}°C</b></div>
          ...
          <div style="margin-top:8px;padding-top:6px;border-top:1px solid #f3f4f6">
            <div style="display:flex;align-items:center;margin-bottom:3px">
              ...
              <span>
                <b>{vent_val} km/h</b>
                <span style="color:#6b7280"> venant du {cp['Dir']}</span>
              </span>
            </div>
            <div style="color:#6b7280;font-size:11px;margin-left:32px">
              Rafales : {cp.get('rafales_val','—')} km/h
            </div>
...
if __name__ == "__main__":
    main()    "⚪ Non classée":    0,   #   0–40   : côte non classée (affichée quand même)
}
...
def creer_carte(points_gpx, resultats_meteo):
    ...
    for cp in resultats_meteo:
        ...
        popup_html = (
            ...
            f"{cp['Ciel']} <b>{t}°C</b><br>"
            f"💧 Pluie : {cp['Pluie']}<br>"
            f"💨 Vent : {cp.get('vent_val','—')} km/h ({cp['Dir']})<br>"
            f"💨 Rafales : {cp.get('rafales_val','—')} km/h<br>"
            f"🚴 Effet : {cp.get('effet','—')}"
        )
        folium.Marker(
            [cp["lat"], cp["lon"]],
